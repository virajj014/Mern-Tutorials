...
// search users
router.post('/searchusers', authTokenHandler, async (req, res) => {
    ...
    // get max 5 users 
    const users = await User.find({ ...keyword, _id: { $ne: req.userId } }).limit(5).select('-password')

    if (!users || users.length === 0) {
        return responseFunction(res, 200, 'Users not found', null, false);
    }
